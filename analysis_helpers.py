...
def mem_gaze_from_df(behavioral_df, eye_df):
    '''
    input:   participant's behavioral df
             participant's eye track df
    output:  df of gaze data when mem image on screen plus the second after it disappears
    '''
    # empty lists for pres_gaze and no_gaze (runs w/ very few recorded datapoints)
    mem_gaze = []

    # for each presentation row in the behavioral df
    for idx,x in behavioral_df[behavioral_df['Trial Type']=='Memory'].iterrows():
        ...
        chunk = eye_df.loc[(eye_df['timestamp']>= start) & (eye_df['timestamp']<=next_sec)]

        # add trial and run numbers
        chunk['Trial'] = np.nan; chunk['Run']   = np.nan; chunk['Trial'] = x['Trial']; chunk['Run']   = x['Run']

        # add start times to separate row
        chunk['Behavior_Image_Start'] = start

        # append the gaze data for each trial to a list
        mem_gaze.append(chunk)

    # concat data from all runs and trials
    mem_gaze = pd.concat(mem_gaze)
